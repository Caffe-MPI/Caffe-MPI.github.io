...
int train() {
  ...
  MPI_Comm_rank (MPI_COMM_WORLD, &rank);
        ...
        if(solver_param.solver_mode() == caffe::SolverParameter_SolverMode_GPU){
                ...
                if(gpunum > 0){
                       if(rank<5) 	
				FLAGS_gpu1 = rank % gpunum;
			else if(rank<10)
				FLAGS_gpu1 = (rank-1) % gpunum;
			else if(rank<15)
				FLAGS_gpu1 = (rank-2) % gpunum;
			else
				FLAGS_gpu1 = (rank-3) % gpunum;
...
// If the gpus flag is not provided, allow the mode and device to be set
  ...
  vector<int> gpus;
  get_gpus(&gpus);
  LOG(INFO)<<"ddddddddddddddddddddddddddddd"<<gpus.size();
  if (gpus.size() == 0) {
    LOG(INFO) << "Use CPU.";
    Caffe::set_mode(Caffe::CPU);
  } else {
...
int main(int argc, char** argv) {
	int provided;
	MPI_Init_thread(&argc,&argv,MPI_THREAD_MULTIPLE,&provided); //added by ZhuHui20151105
