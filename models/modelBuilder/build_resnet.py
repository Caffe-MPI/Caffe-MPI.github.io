...
#------------------------------------------------------------------------------
def addRes_large(model, name , bottom, num_output, group, j, fix_dim, dilation = False):
...
def addResSuperBlock(model, bottom, i, num_subBlocks, num_output, group, fix_dim, net_type, dilation = False):
    ...
    for j in xrange(1, num_subBlocks + 1):
        if net_type=="small":
          ...
          model, top = addRes_large(model, name, bottom=top, num_output=num_output, group=group,
                                    j=j, fix_dim=fix_dim, dilation=dilation)
    return model, top
