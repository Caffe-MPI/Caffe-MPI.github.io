...
void FilterLayer<Ftype, Btype>::LayerSetUp(const vector<Blob*>& bottom,
...
template <typename Ftype, typename Btype>
void FilterLayer<Ftype, Btype>::Reshape(const vector<Blob*>& bottom,
      const vector<Blob*>& top) {
  ...
  const Ftype* bottom_data_selector = bottom[selector_index]->cpu_data<Ftype>();
  ...
  for (int item_id = 0; item_id < bottom[selector_index]->shape(0); ++item_id) {
    ...
    const Ftype* tmp_data_selector = bottom_data_selector + item_id;
...
template <typename Ftype, typename Btype>
void FilterLayer<Ftype, Btype>::Forward_cpu(const vector<Blob*>& bottom,
      const vector<Blob*>& top) {
  int new_tops_num = indices_to_forward_.size();
  // forward all filtered items for all bottoms but the Selector (bottom[last])
  for (int t = 0; t < top.size(); ++t) {
    const Ftype* bottom_data = bottom[t]->cpu_data<Ftype>();
    Ftype* top_data = top[t]->mutable_cpu_data<Ftype>();
...
template <typename Ftype, typename Btype>
void FilterLayer<Ftype, Btype>::Backward_cpu(const vector<Blob*>& top,
    ...
    if (propagate_down[i]) {
      ...
      for (int n = 0; n < bottom[i]->shape(0); n++) {
        ...
        if (next_to_backward_offset >= indices_to_forward_.size()) {
          ...
          caffe_set(dim, Btype(0),
              bottom[i]->mutable_cpu_diff<Btype>() + data_offset_bottom);
        } else {
          ...
          if (n != batch_offset) {  // this data was not been forwarded
            ...
            caffe_copy(dim, top[i]->mutable_cpu_diff<Btype>() + data_offset_top,
                bottom[i]->mutable_cpu_diff<Btype>() + data_offset_bottom);
          }
        }
      }
...
INSTANTIATE_CLASS_FB(FilterLayer);
