...
void ArgMaxLayer<Ftype, Btype>::LayerSetUp(const vector<Blob*>& bottom,
...
template <typename Ftype, typename Btype>
void ArgMaxLayer<Ftype, Btype>::Reshape(const vector<Blob*>& bottom,
      const vector<Blob*>& top) {
...
template <typename Ftype, typename Btype>
void ArgMaxLayer<Ftype, Btype>::Forward_cpu(const vector<Blob*>& bottom,
    const vector<Blob*>& top) {
  ...
  std::vector<std::pair<Ftype, int> > bottom_data_vector(dim);
  for (int i = 0; i < num; ++i) {
    for (int j = 0; j < dim; ++j) {
...
INSTANTIATE_CLASS_FB(ArgMaxLayer);
