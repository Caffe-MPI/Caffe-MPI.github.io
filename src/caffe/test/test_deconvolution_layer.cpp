...
class DeconvolutionLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  DeconvolutionLayerTest() : blob_bottom_(new TBlob<Dtype>(2, 3, 6, 4)),
...
TYPED_TEST(DeconvolutionLayerTest, TestSetup) {
  ...
  shared_ptr<Layer<Dtype, Dtype>> layer(new DeconvolutionLayer<Dtype, Dtype>(layer_param));
  ...
  layer.reset(new DeconvolutionLayer<Dtype, Dtype>(layer_param));
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_->num(), 2);
