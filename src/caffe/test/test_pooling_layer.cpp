...
class PoolingLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  PoolingLayerTest() : blob_bottom_(new TBlob<Dtype>()), blob_top_(new TBlob<Dtype>()),
                       blob_top_mask_(new TBlob<Dtype>()) {}
  ...
  TBlob<Dtype>* const blob_top_mask_;
  ...
  void TestForwardSquare() {
    ...
    PoolingLayer<Dtype, Dtype> layer(layer_param);
...
TYPED_TEST(PoolingLayerTest, TestGradientMax) {
  ...
  for (int kernel_h = 3; kernel_h <= 4; kernel_h++) {
    for (int kernel_w = 3; kernel_w <= 4; kernel_w++) {
      ...
      pooling_param->set_pool(PoolingParameter_PoolMethod_MAX);
      PoolingLayer<Dtype, Dtype> layer(layer_param);
      GradientChecker<Dtype> checker(2e-3, 5e-2);
...
TYPED_TEST(PoolingLayerTest, TestGradientMaxTopMask) {
  ...
  for (int kernel_h = 3; kernel_h <= 4; kernel_h++) {
    for (int kernel_w = 3; kernel_w <= 4; kernel_w++) {
      ...
      this->blob_top_vec_.push_back(this->blob_top_mask_);
      PoolingLayer<Dtype, Dtype> layer(layer_param);
      GradientChecker<Dtype> checker(tol<Dtype>(1e-4, 2e-3), tol<Dtype>(1e-2, 1e-1));
      checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_, this->blob_top_vec_);
...
TYPED_TEST(PoolingLayerTest, TestForwardAve) {
  ...
  EXPECT_EQ(this->blob_top_->width(), 3);
  layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  Dtype epsilon = tol<Dtype>(1e-5, 2e-3);
...
TYPED_TEST(PoolingLayerTest, TestGradientAve) {
  ...
  for (int kernel_h = 3; kernel_h <= 4; kernel_h++) {
    for (int kernel_w = 3; kernel_w <= 4; kernel_w++) {
      ...
      pooling_param->set_pool(PoolingParameter_PoolMethod_AVE);
      PoolingLayer<Dtype, Dtype> layer(layer_param);
      GradientChecker<Dtype> checker(tol<Dtype>(8e-3, 1e-1), 1e-1);
      checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_, this->blob_top_vec_);
    }
  }
}
...
class CuDNNPoolingLayerTest : public GPUDeviceTest<Dtype> {
 protected:
  CuDNNPoolingLayerTest() : blob_bottom_(new TBlob<Dtype>()), blob_top_(new TBlob<Dtype>()),
                            blob_top_mask_(new TBlob<Dtype>()) {}
  ...
  void TestForwardSquare() {
    ...
    CuDNNPoolingLayer<Dtype, Dtype> layer(layer_param);
    layer.SetUp(blob_bottom_vec_, blob_top_vec_);
    EXPECT_EQ(blob_top_->num(), num);
    EXPECT_EQ(blob_top_->channels(), channels);
    EXPECT_EQ(blob_top_->height(), 2);
...
TYPED_TEST(CuDNNPoolingLayerTest, TestGradientMaxCuDNN) {
  for (int kernel_h = 3; kernel_h <= 4; kernel_h++) {
    for (int kernel_w = 3; kernel_w <= 4; kernel_w++) {
      ...
      pooling_param->set_pool(PoolingParameter_PoolMethod_MAX);
      CuDNNPoolingLayer<TypeParam, TypeParam> layer(layer_param);
      GradientChecker<TypeParam> checker(tol<TypeParam>(1e-4, 2e-3), tol<TypeParam>(1e-2, 1e-1));
      checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_, this->blob_top_vec_);
    }
  }
}
