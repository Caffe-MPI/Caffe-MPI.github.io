 ...
 protected:
  ScaleLayerTest()
      : blob_bottom_(new TBlob<Dtype>(2, 3, 4, 5)),
        ...
        blob_bottom_scale_(new TBlob<Dtype>(vector<int>())),
  ...
  TBlob<Dtype>* const blob_bottom_scale_;
...
TYPED_TEST(ScaleLayerTest, TestForwardEltwise) {
  ...
  for (int i = 0; i < count; ++i) {
    EXPECT_NEAR(data[i], in_data_a[i] * in_data_b[i], tol<Dtype>(1e-5, 4e-2));
  }
}
...
TYPED_TEST(ScaleLayerTest, TestForwardScale) {
  ...
  for (int i = 0; i < count; ++i) {
    EXPECT_NEAR(data[i], in_data[i] * scale, tol<Dtype>(1e-5, 1e-2));
  }
}
...
TYPED_TEST(ScaleLayerTest, TestGradientEltwise) {
  ...
  LayerParameter layer_param;
  layer_param.mutable_scale_param()->set_axis(0);
  ScaleLayer<Dtype, Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(tol<Dtype>(1e-2, 1e-1), tol<Dtype>(1e-3, 5e-2));
