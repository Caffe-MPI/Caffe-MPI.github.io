...
class SliceLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  SliceLayerTest() : blob_bottom_(new TBlob<Dtype>(6, 12, 2, 3)), blob_top_0_(new TBlob<Dtype>()),
                     blob_top_1_(new TBlob<Dtype>()), blob_top_2_(new TBlob<Dtype>()) {}
  ...
  TBlob<Dtype>* const blob_top_0_;
  TBlob<Dtype>* const blob_top_1_;
  TBlob<Dtype>* const blob_top_2_;
  vector<Blob*> blob_top_vec_0_, blob_top_vec_1_;
...
TYPED_TEST(SliceLayerTest, TestSetupNum) {
  ...
  SliceLayer<Dtype, Dtype> layer(layer_param);
...
TYPED_TEST(SliceLayerTest, TestTrivialSlice) {
  ...
  for (int i = 0; i < this->blob_bottom_->count(); ++i) {
    EXPECT_EQ(this->blob_bottom_->cpu_data()[i], this->blob_top_0_->cpu_data()[i]);
  }
}
...
TYPED_TEST(SliceLayerTest, TestSliceAcrossChannels) {
  ...
  ASSERT_EQ(this->blob_bottom_->channels() - kSlicePoint1, this->blob_top_2_->channels());
...
TYPED_TEST(SliceLayerTest, TestGradientTrivial) {
  ...
  this->blob_top_vec_0_.resize(1);
  checker.CheckGradientEltwise(&layer, this->blob_bottom_vec_, this->blob_top_vec_0_);
}
...
TYPED_TEST(SliceLayerTest, TestGradientAcrossNum) {
  ...
  SliceLayer<Dtype, Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(tol<Dtype>(1e-2, 1e-1), tol<Dtype>(1e-3, 1e-2));
  checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_, this->blob_top_vec_0_);
}
...
TYPED_TEST(SliceLayerTest, TestGradientAcrossChannels) {
  ...
  SliceLayer<Dtype, Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(tol<Dtype>(5e-2, 1e-1), tol<Dtype>(1e-2, 1e-1));
  checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_, this->blob_top_vec_0_);
}
