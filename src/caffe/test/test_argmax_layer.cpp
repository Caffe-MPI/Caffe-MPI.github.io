...
class ArgMaxLayerTest : public CPUDeviceTest<Dtype> {
 protected:
  ArgMaxLayerTest()
      : blob_bottom_(new TBlob<Dtype>(10, 10, 20, 20)),
...
TYPED_TEST(ArgMaxLayerTest, TestSetup) {
  ...
  ArgMaxLayer<TypeParam, TypeParam> layer(layer_param);
...
//      EXPECT_LE(bottom_data[i * dim + j], max_val);
      ++total_checks;
      if (bottom_data[i * dim + j] > max_val) ++total_failures;
    }
  }
  if (is_precise<TypeParam>()) {
    EXPECT_EQ(total_failures, 0);
  } else {
    EXPECT_LE(total_failures, total_checks / 3);
  }
}

TYPED_TEST(ArgMaxLayerTest, TestCPUMaxVal) {
  if (!is_precise<TypeParam>()) {
    return;  // fp16 not supported by this layer on CPU
  }
  LayerParameter layer_param;
  ...
  for (int i = 0; i < num; ++i) {
    ...
    EXPECT_EQ(bottom_data[i * dim + max_ind], max_val) << "i " << i << " of " << num;
    for (int j = 0; j < dim; ++j) {
      EXPECT_LE(bottom_data[i * dim + j], max_val) << "i " << i << "j " << j;
    }
  }
}
...
//      EXPECT_EQ(j, count);
  ...
  if (is_precise<TypeParam>()) {
    ...
    EXPECT_LE(total_failures, total_checks / 2);
  }
}
